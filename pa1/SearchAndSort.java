...
public class SearchAndSort {
	...
	*/
	...
	public static int countWordsInUnsorted(String[] A, String b){
	...
	*/
	...
	public static int binarySearch(String[] sortedWords, String query, int startIndex, int endIndex){
		...
		if(endIndex >= 1){
			...
			else if((sortedWords[mid].compareTo(query) < 0) && (mid+1<=endIndex)){
	...
	public static int getSmallestIndex(String[] words, String query, int startIndex, int endIndex){
		...
		int test = binarySearch(words, query, startIndex, endIndex);
		if(test == -1){
				return -1;
		}
		else{
			int helo = getSmallestIndex(words, query, startIndex, test-1);
			if(helo == -1){
				return test;
			}
			return helo;
	...
	public static int getLargestIndex(String[] words, String query, int startIndex, int endIndex){
		...
		int newtest = binarySearch(words, query, startIndex, endIndex);
		if(newtest == -1){
			return -1;
		}
		else{
			int helo = getLargestIndex(words, query, newtest+1, endIndex);
			if(helo == -1){
				return newtest;
			}
			else{
		...
		for (int j = 0; j < timingCount; j++) {
			for (int i = 0; i < queryWords.length; i++) {
				...
				*/
				...
				lesser = getSmallestIndex(allWords, queryWords[i], 0, allWords.length-1);
				greater = getLargestIndex(allWords, queryWords[i], 0, allWords.length-1);
